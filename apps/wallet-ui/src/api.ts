...
export interface Wallet {merchant_available?:boolean;merchant_enabled?:boolean;btc_outbound_sats?:string|null;btc_policy?:{max_payment_sats:string;max_daily_sats:string;human_approval_required:boolean}|null;commerce_enabled?:boolean;policy?:{auto_approve_below:string;max_single_payment:string;max_daily_spend:string;max_carrier_msat:string};wallet_name?:string;recipient_address?:string|null;holdings:Holding[];sats:null;network:string}
export interface Plan {merchant_order?:string;recipient?:{identifier:string;authoritative_domain:string};plan_id:string;request:{asset_id:string;amount:string;invoice:string;payment_hash:string;carrier_msat:string;expires_at:number};available_balance:string;policy:{decision:string;reason?:string}}
