...
If an old API/CLI holds a journal, exit it gracefully first. Do not delete its lock.
...
A duplicate journal is refused by the existing lock rather than silently shared.
