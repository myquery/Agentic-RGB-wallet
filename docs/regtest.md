...
| Stale wallet lock after a crash | Confirm no wallet process is running; inspect node status for reserved hashes before removing only `wallet-state.lock`. Preserve the journal. |
